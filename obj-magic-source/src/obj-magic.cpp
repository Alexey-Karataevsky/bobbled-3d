...
int main(int argc, char* argv[]) {
    ...
    vec3 pScale(args.arg('p', "pScale", 1.0f));
    pScale.x *= args.arg(' ', "pScalex", 0.0f);
    pScale.y *= args.arg(' ', "pScaley", 0.0f);
    pScale.z *= args.arg(' ', "pScalez", 0.0f);
	...
	if (true /*analyze*/) { //always find the size
    ...
    if (pScale.x != 0.0f && pScale.y != 0.0f && pScale.z != 0.0f) {
        vec3 size_in = (ubound - lbound);
        scale.x = pScale.x / size_in.x;
        scale.y = pScale.y / size_in.y;
        scale.z = pScale.z / size_in.z;
        
        //out << "Scale overwrite by pScale:         " << toString(scale) << std::endl;
    }
	...
	while (getline(file, row)) {
		...
		std::string tempst;
		if (row.substr(0,2) == "v ") {  // Vertices
			srow >> tempst >> in.x >> in.y >> in.z >> COLOR.x >> COLOR.y >> COLOR.z;
			vec3 old = in;
			in -= center;
			in *= mirror;
			in *= scale;
            ...
            //if (old != in)
				out << "v " << in.x << " " << in.y << " " << in.z << " " << COLOR.x << " " << COLOR.y << " " << COLOR.z << std::endl; //we add color params to vert
